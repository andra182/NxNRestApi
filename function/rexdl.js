...
async function getApk(url) {
     return new Promise((resolve, reject) => {
          if (!/rexdlbox.com/g.test(url)) return resolve({ status: false, message: 'URL Yang Kamu Masukkan Tidak Valid' })
          axios.get(url)
               .then(({ data }) => {
                    ...
                    const password = $('li.dl-key > span > a').attr('title')
                    let name = []
                    let url_download = []
                    let link_download = []
                    let promiss = []
     ...
     return new Promise((resolve, reject) => {
          axios.get(`https://rexdl.com/?s=${apkname}`)
               .then(({ data }) => {
                    ...
                    $('div > div.post-thumbnail > a').get().map((rest) => {
                         url_download.push('https://rexdlbox.com/index.php?id=' + $(rest).attr('href').split('/')[4].replace('.html', ''))
                    })
